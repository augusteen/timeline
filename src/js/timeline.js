function timeline(collection, options) {
    ...
    function setHeightandWidths(tl) {
        ...
        function setHeights() {
            ...
            tl.items.filter((item) => !item.classList.contains("abs")).forEach((item, i) => {
            ...
            tl.items.filter((item) => item.classList.contains("abs")).forEach((item, i) => {
                ...
                if (i % 2 === 0) {
                    ...
                    if (tl.settings.horizontalStartPosition !== 'bottom') {
                        item.classList.add('timeline__item--bottom');
                        transformString2 = `translate(${i*tl.itemWidth}px,${evenIndexTallest}px)`;
                        addTransforms(item, transformString2);
                    } else {
                        item.classList.add('timeline__item--top');
                        transformString2 = `translateX(${i*tl.itemWidth}px)`;
                        addTransforms(item, transformString2);
                    }
                } else {
    ...
    function timelinePosition(tl) {
        const position = tl.items.filter((item) => !item.classList.contains("abs"))[currentIndex].offsetLeft;
    ...
    function slideTimeline(tl) {
        ...
        const maxIndex = tl.items.filter((item) => !item.classList.contains("abs")).length - tl.settings.visibleItems;
